const scheduleAdd = (function () {
	...
	function init(){
		...
		// schedule_id, product_id가 있으면 편성표 분리
	...
	// 이벤트 초기화 
	function _evInit() {
		let evo = $("[data-src='scheduleAdd'][data-act]").off();
	...
	function _action(ev){
		...
		if(type_v == "click") {
			...
			} else if(act_v == "clickAddCancle") {
				event.clickAddCancle();
			} else if(act_v == "clickAddSchedule") {
				event.clickAddSchedule();
			}
	...
	const _data = {
		...
		getRemainProductList: function() {
			...
			if(_urlParam && _urlParam.product_id) {
				data_v.except_product_id = _urlParam.product_id;
			}
			...
			if(slotType == "C") {
				...
				if(data.sg_id) {
					...
					$(slot_o).attr({
						...
						"data-matching-sg-id": data.sg_id,
						"data-separated-schedule-product-id": data.schedule_product_id,
					});
				}
		...
		getSubmitData: function() {
			...
			// 상품 아이디 리스트
			data.product_id_list = _productFilterList;
			
			// 편성표 분리 시
			if(_urlParam && _urlParam.product_id){
				
				// 분리된 상품을 삭제하고 편성표를 등록할 경우 일반 등록이 되어야 한다.
				// 분리된 상품이 추가 상품 리스트에 있는지 체크
				if(_productFilterList.includes(_urlParam.product_id)) {
					data.separated_product_id = _urlParam.product_id;
				}
			}
		...
		getSlotDataList: function() {
			...
			$("#slotList").children("li").each(function(i, li_o){
				...
				let separatedScheduleProductId = $(li_o).attr("data-separated-schedule-product-id");
				let matchingSgId = $(li_o).attr("data-matching-sg-id");
				
				// 슬롯에 묶인 분리된 상품과 광고 아이디 정보
				if(separatedScheduleProductId) {
					slotData.slot_id = Number(slotId);
					slotData.separated_schedule_product_id = Number(separatedScheduleProductId);
					slotData.matching_sg_id = Number(matchingSgId);
				}
	...
	const _event = {
		...
		clickRemoveProduct: function(evo) {
			...
			evo.parent().parent().remove();
		}, 
		
		// 상품 등록
		clickAddSchedule: function() {
			...
			let url_v = "/campaign/schedule/add";
