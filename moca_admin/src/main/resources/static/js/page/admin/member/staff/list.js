const staffList = (function(){
	...
	function _action(ev){
		...
		if(type_v == "click"){
			...
			//직원 개별 권한 수정
			}else if(act_v == "modifyMemberRole"){
				event.modifyMemberRole(evo);
			//직원 삭제 이벤트
	...
	let _event = {
		...
		setStaffModiyModal: function(evo){
			...
			if(roleName != "최고 관리자"){
				...
				let url_v = "/role/staff/permission/menu/list";
			
			
				$("#modifyModalRoleSelect").selectpicker("val", $(evo).attr("data-role-id"));
				
				$("#modfiyMemberRoleBnt").attr({
				...
				comm.send(url_v, data_v, "POST", function(resp){
					//구분 그리기
					_list.drawRoleMenuList(resp.list[0], "staffDetailModalMenuBody");
				});
			}else{
				customModal.alert({
			...
			comm.send(url_v, data_v, "POST", function(resp){
				...
				if(resp.result){
					...
					if(resp.code == 2200){
						...
						msg = "아이디는 영문과 숫자를 포함해 6자 이상 15자 이하로 입력해 주세요.";
					}
					customModal.alert({
						content: msg
					});
		...
		removeStaff: function(){
			...
			comm.send(url_v, data_v, "POST", function(resp){
				if(resp.result){
					...
					}else if(resp.code ==2400){
						msg = "최고 관리자는 삭제할 수 없습니다.";
					}
					customModal.alert({
						content: msg
					});
		...
		//sorting된 리스트 저장
		modifySort: function(){
			let url_v = "/role/staff/modify/sort";
		...
		clickRoleNameBtn: function(){
	        ...
	        $('#roleNameSubmit, #inputRoleName').show();  
		...
		modifyRoleName: function(){
			...
			let url_v = "/role/staff/modify/name";
		...
		//직원 구분 변경
		modifyMemberRole: function(evo){
			let url_v = "/member/staff/role/modify";
			let data_v = {};
	...
	let _list = {
		...
		getSearchData: function(){
			...
			if(searchText_v) {
				data.search_value = searchText_v;
			}
		...
		getModifyHistoryList: function(evo){
			let url_v = "/member/staff/history/list";
			let data_v = {
			...
			comm.send(url_v,data_v, "POST", function(resp){
				_list.drawMgetModifyHistoryList(resp.list, uid, uname);
			});
		},
		//더보기 그려주는 이벤트
		drawMgetModifyHistoryList: function(list, id, name){
			let uid = name + "(" + id + ")";
