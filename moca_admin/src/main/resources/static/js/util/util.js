const util = (function(){
	...
	function staffLogin({
		...
		if(staffUtype != "A" && staffUtype != "B") return;
		
		let url_v = "/member";
		let data_v = {};
		
		if(staffUtype == globalConfig.memberType.ADMIN.utype) {
			url_v += "/staff/login";
			data_v.member_id = memberId;
		} else if(staffUtype == globalConfig.memberType.AGENCY.utype) {
		...
		customModal.confirm({
			...
			confirmCallback: () => {
				comm.send(url_v, data_v, "POST", function(resp){
					if(resp.result) {
						let url = resp.return_url;
						url += "/login?temp_auth_value=" + resp.temp_auth_value + "&member_id=" + memberId;;
						if(callbackUrl) {
							url += "&returnUrl=" + callbackUrl;
						}
