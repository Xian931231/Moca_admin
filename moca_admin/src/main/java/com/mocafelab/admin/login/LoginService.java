package com.mocafelab.admin.login;
...
public class LoginService {
	...
	private Map<String, Object> loginProcess(Map<String, Object> param, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(CommonUtil.checkIsNull(loginData)) {
			...
			// 로그인 가능 여부 검사
			code = isLoginEnable(param, loginData, false);
	 ...
	 * @throws Exception
	 */
	public Code isLoginEnable(Map<String, Object> param, Map<String, Object> loginData, boolean isAdmin) throws Exception {
		...
		if(!MemberType.getType(utype).equals(MemberType.ADMIN) && !isAdmin) {
			// 관리자가 아닌 경우 로그인 불가
