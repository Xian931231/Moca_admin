package com.mocafelab.admin.member;
...
import com.mocafelab.admin.alarm.AlarmData;
import com.mocafelab.admin.alarm.AlarmSender;
import com.mocafelab.admin.email.EmailService;
import com.mocafelab.admin.email.template.ModifyInfoCompleteTemplate;
import com.mocafelab.admin.email.template.SignUpAuthTemplate;
import com.mocafelab.admin.email.template.SignUpCompleteTemplate;
import com.mocafelab.admin.email.template.StaffTempPasswordTemplate;
import com.mocafelab.admin.email.template.SupplyApproveTemplate;
import com.mocafelab.admin.email.template.TempPasswordTemplate;
import com.mocafelab.admin.enums.MemberStatus;
import com.mocafelab.admin.enums.MemberType;
import com.mocafelab.admin.enums.ModifyHistory;
import com.mocafelab.admin.file.S3Service;
import com.mocafelab.admin.login.LoginMapper;
import com.mocafelab.admin.login.LoginService;
import com.mocafelab.admin.login.SessionUtil;
import com.mocafelab.admin.menu.MenuService;
import com.mocafelab.admin.role.RoleMapper;
...
public class MemberService {
	...
	private String PROJECT_WEB_URL;
	
	@Value("${staff.login.expire.second}")
	private String STAFF_LOGIN_EXPIRE_SECOND;
...
//		}
	 ...
	 * @throws Exception
	 */
	public Map<String, Object> memberLoginByStaff(Map<String, Object> param, HttpServletRequest request, HttpServletResponse response) throws Exception {
		ResponseMap respMap = beanFactory.getResponseMap();
		
		// 해당 API 호출자가 관리자 계정인지 확인
		String loginUtype = (String) param.get("login_utype");
		if(!loginUtype.equals(MemberType.ADMIN.getType())) {
			return respMap.getResponse(Code.MEMBER_ROLE_NOT_ACCEPT);
		}
		...
		// 관리자 계정인 경우는 막음. TODO 수정 필요할지도..
		String memberUType = (String) memberData.get("utype");
		if(MemberType.getType(memberUType).equals(MemberType.ADMIN)) {
			return respMap.getResponse(Code.LOGIN_FAIL);
		}
		
		Code code = Code.OK;
		
		code = loginService.isLoginEnable(param, memberData, true);
		
		if(code.equals(Code.OK)) {
			// 임시 유효 값 저장
			String tempAuthValue = SHAUtil.encrypt(UUID.randomUUID().toString());
			param.put("temp_auth_value", tempAuthValue);
			param.put("temp_auth_expire_second", STAFF_LOGIN_EXPIRE_SECOND);
			if(loginMapper.setTempAuthValue(param) > 0) {
				respMap.setBody("temp_auth_value", tempAuthValue);
				respMap.setBody("return_url", PROJECT_WEB_URL);
			}
		} else {
			return respMap.getResponse(Code.LOGIN_FAIL);
		}
		...
		/*
		URL url = new URL(PROJECT_WEB_URL + "/api/v1/member/staff/login");
		...
		paramInfo.put("login_access_token", sessionUtil.getUserTokenInSession(session));
		paramInfo.put("login_member_id", sessionUtil.getLoginIdInSession(session));
		paramInfo.put("member_id", param.get("member_id"));
		
		String jsonData = CommonUtil.mapToJson(paramInfo);
		
		AES256Util aes256Util = new AES256Util(SECRET_DEFAULT_KEY);
		jsonData = "e=" + UriEncoder.encode(aes256Util.encrypt(jsonData));
		...
		Map<String, Object> responseData = CommonUtil.jsonToMap(sb.toString());
		if(!CommonUtil.checkIsNull(responseData)) {
			String eParam = (String) responseData.get("e");
			String eData = aes256Util.decrypt(eParam);
			Map<String, Object> eMap = CommonUtil.jsonToMap(eData);
			
			if(!CommonUtil.checkIsNull(eMap)) {
				if(((Map<String, Object>) eMap.get("header")).get("code").equals(200)) {
					
					Map<String, Object> body = (Map<String, Object>) eMap.get("body"); 
					if(!CommonUtil.checkIsNull(body) && !CommonUtil.checkIsNull(body, "temp_auth_value")) {
						respMap.setBody("temp_auth_value", body.get("temp_auth_value"));
						respMap.setBody("return_url", PROJECT_WEB_URL);
						return respMap.getResponse();
					}
				}
	...
	public void getMemberUserExcel(Map<String, Object> param,  HttpServletResponse response) throws Exception{
		...
		List<Map<String, Object>> memberList = memberMapper.getMemberUserExcel(param);
