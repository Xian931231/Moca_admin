...
public interface ExternalDeviceMapper {
	...
	// 디바이스 삭제
	public int removeDevice(int param);
	// 디바이스 시리얼넘버 중복 체크 
	...
	// 측정 장비 시리얼 번호 삭제
	public int removeSensorDevice(int param);
	// 장비에 속하는 측정 장비인지 확인
	...
	// 게재 위치 구분 아이디 삭제
	public int removeMotorPositionId(int param);
	// 게재 위치 구분 아이디 상세
	public Map<String, Object> getMotorPositionIdDetail(Map<String, Object> param);
	// 게재 위치와 매칭된 디바이스 확인
	public Map<String, Object> hasMatchingDevice(int param);
	// 개제 위치와 매칭된 상품 기기 해제
	public int modifyDeviceMotorIdToNull(int param);
	// 개제 위치와 매칭된 측정 장비 해제
	public int modifySensorMotorIdToNull(int param);
